...
class RequestFuncOutput:
    ...
    itl: List[float] = field(
        default_factory=list)  # List of per-chunk inter-token latencies
    text_chunks: List[str] = field(
        default_factory=list)  # Text of each streamed chunk (for ITL re-distribution)
                        ...
                        if chunk != "[DONE]":
                            ...
                            if choices := data.get("choices"):
                                ...
                                # Decoding phase
                                    ...
                                    output.itl.append(timestamp -
                                                      most_recent_timestamp)
                                    # Record the chunk text so ITL can later be
                                    # re-distributed per token. With speculative
                                    # decoding / MTP a single chunk may carry
                                    # several tokens, otherwise per-chunk ITL
                                    # over-counts inter-token latency.
