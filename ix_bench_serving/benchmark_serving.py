...
def calculate_metrics(
    ...
    for i in range(len(outputs)):
        if outputs[i].success:
            ...
            # ITL is recorded once per streamed chunk, but with speculative
            # decoding / MTP a single chunk can carry several tokens. Counting
            # one ITL per chunk over-states inter-token latency by roughly the
            # acceptance length. When per-chunk text is available, re-tokenize
            # each chunk and spread its latency evenly across its tokens.
