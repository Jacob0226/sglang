...
def _tree_target_only_reference(
    *,
    ...
    uniform_samples_for_final_sampling = (
        uniform_samples_for_final_sampling.cpu()
    )
    ...
    accept_index = torch.full(
        (batch_size, max_tree_depth), -1, dtype=torch.int32
    )
    ...
    for batch_idx in range(batch_size):
        ...
        for _ in range(1, max_tree_depth):
            current_tree_idx = int(
                retrieve_next_token[batch_idx, current_tree_idx]
            )
            ...
            while current_tree_idx != -1:
                ...
                if accept_draft:
                    draft_global_idx = int(
                        retrieve_index[batch_idx, current_tree_idx]
                    )
                    predicts[last_accept_global_idx] = draft_token
                    num_correct_drafts[batch_idx] += 1
                    accept_index[
                        batch_idx, int(num_correct_drafts[batch_idx])
                    ] = draft_global_idx
                    last_accept_global_idx = draft_global_idx
                    current_prob_row = current_tree_idx
                    coin = float(
                        uniform_samples[batch_idx, current_tree_idx]
                    )
                    break

                draft_probs[
                    batch_idx, current_prob_row, draft_token
                ] = target_prob
                current_tree_idx = int(
        ...
        all_drafts_accept = (
            int(num_correct_drafts[batch_idx]) == max_tree_depth - 1
        )
        ...
        target_cdf = (
            float(uniform_samples_for_final_sampling[batch_idx])
            * float(residual.sum())
        )
