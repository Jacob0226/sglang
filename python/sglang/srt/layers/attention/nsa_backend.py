...
class NativeSparseAttnBackend(
    ...
    def forward_decode(
        ...
        if q_rope is not None:
            ...
            # Caller passed split q_nope / q_rope; we need to concat below if
            # the chosen impl wants q_all.
            q_all = None
        else:
            # Caller passed the already-concatenated q (q_all = q). We can use it
            # directly via zero-copy slicing; the impl-specific blocks below
            # will skip the otherwise redundant concat_mla_absorb_q_general call.
            q_all = q.contiguous().view(-1, layer.tp_q_head_num, layer.head_dim)
            q_nope = q_all[:, :, : layer.v_head_dim]
            q_rope = q_all[:, :, layer.v_head_dim :]
        ...
        # Cat-skip optimization is HIP-only: when caller passes q_rope=None on
        # HIP, q_all has already been set to a zero-copy view of q in the else
        # branch above and we can reuse it directly. Non-HIP backends fall
        # through to the original "always cat" behavior to keep CUDA / MUSA
